...
class HeapNode<S extends Comparable<S>, T extends Comparable<T>>{
    ...
    public int compareHeapNode(S new_value1, T new_value2) {
    	if(this.value2.compareTo(new_value2) >= 1) {
    		return 1;
    	}
    	else if (this.value2.compareTo(new_value2) == 0) {
    		return 0;
    	}
    	return -1;
    }
...
{    
    ...
    public int compareAVLNode(S new_value1, T new_value2) {
    	if(this.value1.compareTo(new_value1) >= 1) {
    		return 1;
    	}
    	else if(this.value1.compareTo(new_value1) == 0) {
    		return 0;
    	}
    	return -1;
    }
...
public class GPADatabase {
	...
	public GPADatabase(int size) {
		this_heap = new MaxHeap<String,Double>(size);
		this_avltree = new AVLTree<String, Double>();
	}	
	...
	public void insertStudent(String student_name, Double gpa) {
		this_avltree.insert(student_name, gpa);
		HeapNode temp = new HeapNode(student_name, gpa);
		this_heap.add(temp);
	...
	public ArrayList<String> removeTopkStudent(int k) {
			...
			while(z > 0) {
				if(this_heap.toString().length() == 5) {
					return m;
				}
				
				HeapNode temp = this_heap.poll(); 
				this_avltree.deleteNode(temp.value1, temp.value2);
				m.add((String) temp.value1);
				z--;
			}
		return m;
	}
	...
	public Double searchStudentGPA(String student_name) {
		...
		/***while(this_avltree != null) {
			...
			//System.out.println("avl tree: " + this_avltree.toString());
			
			//System.out.println("heap tree: " + this_heap.toString());
			
			//System.out.println("Heap lenght " + (this_heap.toString().length()));
			
			//System.out.println("temp lenght " + temp.toString().length());
			
			//AVLTree temp2 = this_avltree;
			
			//temp2.deleteNode("C", 0.0 );
			
			//System.out.println("avl tree: " + this_avltree.toString().length());
			
			if(this_heap.toString().length() == 5) {
				//System.out.println("true");
				return null;
			}
			if(temp.value1 == student_name) {
				return (Double) temp.value2;
			}
		}***/
		
		while(this_heap.toString().length() != 5) {
			if(this_heap.peek().value1.equals(student_name)) {
				System.out.println("True");
				return (double)this_heap.peek().value2;
			}
			this_heap.poll();
		}
	...
	public static void main(String[] args) {
		...
		GPADatabase temp = new GPADatabase(0);
   	...
   	System.out.println(temp.removeTopkStudent(10));
